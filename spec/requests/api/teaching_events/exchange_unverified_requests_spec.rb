...
RSpec.describe "POST /api/teaching_events/attendees/exchange_unverified_request", type: :request do
  ...
  before do
    allow(teaching_events_resource).to receive(:exchange_unverified_request).and_return(response_data)
  end
  ...
  describe "when the CRM is unavailable" do
    before do
      allow(teaching_events_resource).to receive(:exchange_unverified_request)
