...
RSpec.describe TeacherTrainingAdviser::ExchangeAccessToken do
  ...
  describe "#call" do
    before do
      allow(crm_client).to receive(:teacher_training_adviser).and_return(tta_resource)
      allow(tta_resource).to receive(:exchange_access_token).and_return(true)
    end

    context "when valid" do
      it "calls the CRM client with the token and camelized body" do
        subject.call
        expect(tta_resource).to have_received(:exchange_access_token) do |token, body|
      ...
      it "excludes access_token from the body" do
        subject.call
        expect(tta_resource).to have_received(:exchange_access_token) do |_token, body|
    ...
    context "when invalid" do
      ...
      it "returns false without calling the CRM client" do
        subject.call
        expect(tta_resource).not_to have_received(:exchange_access_token)
      end
