...
RSpec.describe GetIntoTeaching::Matchback do
  ...
  describe "#create" do
    before do
      ...
      allow(get_into_teaching_resource).to receive(:matchback).and_return(true)
    end

    context "when valid" do
      it "calls the CRM client with the camelized body" do
        subject.create
        expect(get_into_teaching_resource).to have_received(:matchback) do |body|
    ...
    context "when invalid" do
      ...
      it "returns false without calling the CRM client" do
        subject.create
        expect(get_into_teaching_resource).not_to have_received(:matchback)
      end
