...
RSpec.describe "GET /api/pick_list_items/candidate/consideration_journey_stages", type: :request do
  ...
  describe "response format" do
    it "returns JSON" do
      get(api_pick_list_items_candidate_consideration_journey_stages_path, headers:)
      expect(response.content_type).to match(%r{application/json})
    end

    it "returns JSON even when the client requests HTML" do
      get api_pick_list_items_candidate_consideration_journey_stages_path, headers: headers.merge({ "Accept" => "text/html" })
